...
def parse_excel(file_path):
    ...
    for index, row in df.iterrows():
        ...
        if pd.notna(okpd2_value):
            data[current_entity]['objects'][current_object]['ОКПД2'] = {'value': okpd2_value, 'additional_value': None}
        ...
        if ktru_text:
            data[current_entity]['objects'][current_object]['КТРУ текст'] = {'value': ktru_text, 'additional_value': None}
        if ktru_hyperlink:
            data[current_entity]['objects'][current_object]['КТРУ ссылка'] = {'value': ktru_hyperlink, 'additional_value': None}
        if kkn_text:
            data[current_entity]['objects'][current_object]['ККН текст'] = {'value': kkn_text, 'additional_value': None}
        if kkn_hyperlink:
            data[current_entity]['objects'][current_object]['ККН ссылка'] = {'value': kkn_hyperlink, 'additional_value': None}
        ...
        value = row.iloc[8] if pd.notna(row.iloc[8]) else None
        additional_value = row.iloc[9] if pd.notna(row.iloc[9]) else None
        date_actualization = row.iloc[15] if pd.notna(row.iloc[15]) else None  # Дата актуализации
        russian_product = row.iloc[16] if pd.notna(row.iloc[16]) else None  # Российский товар
        # Формируем значение
        ...
        if value is not None or additional_value is not None:
            ...
            data[current_entity]['objects'][current_object][characteristic] = {
                'value': value,
                'additional_value': additional_value,
                'date_actualization': date_actualization,
                'russian_product': russian_product
            }
    return data
    ...
    try:
        ...
        for entity_name, entity_data in tqdm(data.items(), desc="Импорт сущностей"):
            ...
            for obj_name, obj_data in entity_data['objects'].items():
                ...
                for char_name, char_value in obj_data.items():
                    if char_name == 'ОКПД2':
                    ...
                    cur.execute("""
                        INSERT INTO object_values (object_id, characteristic_id, value, additional_value, date_actualization, russian_product)
                        VALUES (%s, %s, %s, %s, %s, %s)
                        ON CONFLICT (object_id, characteristic_id)
                        DO UPDATE SET value = EXCLUDED.value, additional_value = EXCLUDED.additional_value, date_actualization = EXCLUDED.date_actualization, russian_product = EXCLUDED.russian_product
                    """, (obj_id, char_id, char_value.get('value'), char_value.get('additional_value'), char_value.get('date_actualization'), char_value.get('russian_product')))
        conn.commit()
